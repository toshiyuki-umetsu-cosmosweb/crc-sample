...
static void generate_crc_table(crc8_calculator_t *calculator);
 ...
 */
void
crc8_calculator_init(crc8_calculator_t *calculator,
        uint8_t initial_value, uint8_t polynomial, uint8_t final_xor_value,
 ...
 */
static void
generate_crc_table(crc8_calculator_t *calculator)
{
    for (uint32_t divident = 0u; divident < 256u; divident++) {
        uint8_t value = divident;
 ...
 */
void
crc8_calculator_reset(crc8_calculator_t *calculator)
{
    if (calculator != NULL) {
 ...
 * @return Current result returned. (same as crc8_calculator_get())
 */
uint8_t
crc8_calculator_append(crc8_calculator_t *calculator,
        const void *data, size_t len)
{
    if ((data != NULL) && (len > 0)) {
        const uint8_t *input_data = (const uint8_t*)(data);
        for (uint32_t n = 0u; n < len; n++) {
            uint8_t crc = (*calculator).crc;
            uint8_t d = (*calculator).is_input_reflected
                    ? swap_ui8(input_data[n]) : input_data[n];
            uint8_t pos = crc ^ d;
            (*calculator).crc = (*calculator).table[pos];
        }
    }

    return crc8_calculator_get(calculator);
}
/**
 * Get CRC result.
 *
 ...
 */
uint8_t
crc8_calculator_get(const crc8_calculator_t *calculator)
{
    uint8_t retval;

    if (calculator != NULL) {
        uint8_t  value = (*calculator).crc ^ (*calculator).final_xor_value;
        if ((*calculator).is_result_reflected) {
            retval = swap_ui8(value);
        } else {
            retval = value;
        }
    } else {
 ...
 */
uint8_t
crc8_calculate(const void *data, size_t len,
